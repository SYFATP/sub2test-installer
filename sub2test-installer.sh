...
def mark_account_inactive_error(account_id: int):
    return update_account_fields(account_id, {
        'status': 'inactive',
        'schedulable': False,
    })
...
def keep_account_inactive(account_id: int):
    return mark_account_inactive_error(account_id)
...
def run_account_test(row):
    ...
    if source_status == 'inactive' and native_status == 'success':
        ...
        if native_status == 'token_expired':
            ...
            keep_inactive_success, keep_inactive_status, keep_inactive_detail = mark_account_inactive_error(int(account_id))
